...
func Root() *cobra.Command {
	app := &App{cfg: dictionary.DefaultConfig()}

	root := &cobra.Command{
		Use:   "dict",
		Short: "Look up word definitions, synonyms and phonetics",
		Long: `dict reads word definitions, phonetics, synonyms, antonyms, and examples
from the Free Dictionary API (api.dictionaryapi.dev). The API is completely
free and requires no authentication. Records are returned as table, JSON,
JSONL, CSV, TSV, or raw.

dict is an independent tool and is not affiliated with any commercial dictionary.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVar(&app.lang, "lang", "en", "language code (en, es, fr, de, ...)")
...
func (a *App) setup() error {
	...
	a.client = dictionary.NewClient(a.cfg)
	return nil
}
