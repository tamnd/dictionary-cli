...
describes what users get. Pick a format with `-o`, or let dict choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dict <command> -o table   # aligned columns for reading
dict <command> -o jsonl   # one JSON object per line, for piping
dict <command> -o json    # a single JSON array
dict <command> -o csv     # spreadsheet friendly
dict <command> -o tsv     # tab-separated
dict <command> -o url     # just the URL column
dict <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dict <command> --fields id,title,url
```
...
```bash
dict <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dict <command>            # a table, because this is a terminal
dict <command> | wc -l    # JSONL, because this is a pipe
```
